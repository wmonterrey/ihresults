				...
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
						<li>
							...
							<i class="fa fa-angle-right"></i> <a href="<spring:url value="/super/studies/" htmlEscape="true "/>"><spring:message code="superstudies" /></a>
						</li>
					</ul>
					...
					<div class="portlet">
						...
						<div class="portlet-body">
							<div class="table-toolbar">
								<div class="btn-group">
									<spring:url value="/super/studies/newStudy"	var="newStudy"/>
									<button id="lista_estudios_new" onclick="location.href='${fn:escapeXml(newStudy)}'" class="btn btn-success">
							...
							<c:forEach items="${estudios}" var="estudio">
								<tr>
									<spring:url value="/super/studies/viewStudy/{study}"
										var="estudioUrl">
										<spring:param name="study" value="${estudio.idEstudio}" />
									</spring:url>
									<spring:url value="/super/studies/editStudy/{study}"
										var="editUrl">
										<spring:param name="study" value="${estudio.idEstudio}" />
									</spring:url>
									<spring:url value="/super/studies/act/disable1/{study}"
										var="disableUrl">
										<spring:param name="study" value="${estudio.idEstudio}" />
									</spring:url>
									<spring:url value="/super/studies/act/enable1/{study}"
										var="enableUrl">
										<spring:param name="study" value="${estudio.idEstudio}" />
									</spring:url>
									...
									<td>
										<a href="${fn:escapeXml(estudioUrl)}" class="btn btn-default btn-xs"><i class="fa fa-search"></i></a>
										<a href="${fn:escapeXml(editUrl)}" class="btn btn-default btn-xs"><i class="fa fa-edit"></i></a>
										<c:choose>
										<c:when test="${estudio.pasive=='0'.charAt(0)}">
											<a data-toggle="modal" data-estudio="${estudio.nombreEstudio}" data-id= "${fn:escapeXml(disableUrl)}" class="btn btn-default btn-xs desact"><i class="fa fa-trash-o"></i></a>
										</c:when>
										<c:otherwise>
											<a data-toggle="modal" data-estudio="${estudio.nombreEstudio}" data-id= "${fn:escapeXml(enableUrl)}" class="btn btn-default btn-xs act"><i class="fa fa-check"></i></a>
										</c:otherwise>
										</c:choose>
									</td>
								</tr>
...
<script>
	jQuery(document).ready(function() {
		App.init();
		var table  = $('#lista_estudios').DataTable({
