				...
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
						<li>
							...
							<i class="fa fa-angle-right"></i> <a href="<spring:url value="/super/accounts/" htmlEscape="true "/>"><spring:message code="superaccounts" /></a>
						</li>
					</ul>
					...
					<div class="portlet">
						...
						<div class="portlet-body">
							<div class="table-toolbar">
								<div class="btn-group">
									<spring:url value="/super/accounts/newAccount"	var="newAccount"/>
									<button id="lista_cuentas_new" onclick="location.href='${fn:escapeXml(newAccount)}'" class="btn btn-success">
							...
							<c:forEach items="${cuentas}" var="cuenta">
								<tr>
									<spring:url value="/super/accounts/viewAccount/{account}"
										var="cuentaUrl">
										<spring:param name="account" value="${cuenta.idCuenta}" />
									</spring:url>
									<spring:url value="/super/accounts/editAccount/{account}"
										var="editUrl">
										<spring:param name="account" value="${cuenta.idCuenta}" />
									</spring:url>
									<spring:url value="/super/accounts/act/disable1/{account}"
										var="disableUrl">
										<spring:param name="account" value="${cuenta.idCuenta}" />
									</spring:url>
									<spring:url value="/super/accounts/act/enable1/{account}"
										var="enableUrl">
										<spring:param name="account" value="${cuenta.idCuenta}" />
									</spring:url>
									...
									<td>
										<a href="${fn:escapeXml(cuentaUrl)}" class="btn btn-default btn-xs"><i class="fa fa-search"></i></a>
										<a href="${fn:escapeXml(editUrl)}" class="btn btn-default btn-xs"><i class="fa fa-edit"></i></a>
										<c:choose>
										<c:when test="${cuenta.pasive=='0'.charAt(0)}">
											<a data-toggle="modal" data-cuenta="${cuenta.nombreCuenta}" data-id= "${fn:escapeXml(disableUrl)}" class="btn btn-default btn-xs desact"><i class="fa fa-trash-o"></i></a>
										</c:when>
										<c:otherwise>
											<a data-toggle="modal" data-cuenta="${cuenta.nombreCuenta}" data-id= "${fn:escapeXml(enableUrl)}" class="btn btn-default btn-xs act"><i class="fa fa-check"></i></a>
										</c:otherwise>
										</c:choose>
									</td>
								</tr>
...
<script>
	jQuery(document).ready(function() {
		...
		$(".act").click(function(){ 
        	...
        	$('#cuerpo').html('<h3>'+"${habilitar}"+' '+$(this).data('cuenta')+'?</h3>');
        	$('#basic').modal('show');
        });
        ...
        $(".desact").click(function(){ 
        	$('#accionUrl').val($(this).data('id'));
        	$('#titulo').html('<h2 class="modal-title">'+"${confirmar}"+'</h2>');
        	$('#cuerpo').html('<h3>'+"${deshabilitar}"+' '+$(this).data('cuenta')+'?</h3>');
        	$('#basic').modal('show');
        });
