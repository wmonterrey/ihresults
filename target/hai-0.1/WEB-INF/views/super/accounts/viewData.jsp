...
<!-- BEGIN CONTENT -->
<c:set var="accountEnabledLabel"><spring:message code="account.enabled" /></c:set>
<c:set var="accountDisabledLabel"><spring:message code="account.disabled" /></c:set>
<c:set var="habilitar"><spring:message code="enable" /></c:set>
<c:set var="deshabilitar"><spring:message code="disable" /></c:set>
<c:set var="confirmar"><spring:message code="confirm" /></c:set>

<spring:url value="/super/accounts/editAccount/{account}"	var="editUrl">
	<spring:param name="account" value="${account.idCuenta}" />
</spring:url>
<spring:url value="/super/accounts/act/disable2/{account}" var="disableUrl">
	<spring:param name="account" value="${account.idCuenta}" />
</spring:url>
<spring:url value="/super/accounts/act/enable2/{account}"	var="enableUrl">
	<spring:param name="account" value="${account.idCuenta}" />
</spring:url>
		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-md-12">
					...
					<h3 class="page-title">
					<spring:message code="title" /> <small><spring:message code="superaccounts" /></small>
					</h3>
					<ul class="page-breadcrumb breadcrumb">
						<li>
							<i class="fa fa-home"></i>
							<a href="<spring:url value="/" htmlEscape="true "/>"><spring:message code="home" /></a>
							<i class="fa fa-angle-right"></i> <a href="<spring:url value="/super/accounts/" htmlEscape="true "/>"><spring:message code="superaccounts" /></a> <i class="fa fa-angle-right"></i> <a href="<spring:url value="/super/accounts/viewAccount/${account.idCuenta}" htmlEscape="true "/>">${account.idCuenta}</a>
						</li>
					</ul>
					...
					<div class="portlet">
						...
						<div class="portlet-body">
							<div class="table-toolbar">
								<div class="btn-group">
									<spring:url value="/super/accounts/"	var="listAccount"/>
									<button id="lista_accounts" onclick="location.href='${fn:escapeXml(listAccount)}'" class="btn btn-info">
									<spring:message code="superaccounts" /> <i class="fa fa-reply"></i>
									</button>
								</div>
								<div class="btn-group pull-right">
									...
									<ul class="dropdown-menu pull-right">
										...
										<c:choose>
											<c:when test="${account.pasive=='0'.charAt(0)}">
												<li>
													<a data-toggle="modal" data-account="${account.nombreCuenta}" data-id= "${fn:escapeXml(disableUrl)}" class="desact"><i class="fa fa-trash-o"></i> <spring:message code="disable" /></a>
												</li>
											</c:when>
											<c:otherwise>
												<li>
													<a data-toggle="modal" data-account="${account.nombreCuenta}" data-id= "${fn:escapeXml(enableUrl)}" class="act"><i class="fa fa-check"></i> <spring:message code="enable" /></a>
												</li>
											</c:otherwise>
										</c:choose>
									</ul>
							...
							<form class="form-horizontal">
								<div class="form-body">
									<h3 class="form-section"><spring:message code="idCuenta" />: <c:out value="${account.idCuenta}" /></h3>
									<div class="row">
										<div class="col-md-6">
											<div class="form-group">
												<label class="control-label col-md-3"><spring:message code="nombreCuenta" />:</label>
												<div class="col-md-9">
													<p class="form-control-static">
														 <c:out value="${account.nombreCuenta}" />
													</p>
												</div>
											</div>
										</div>
									...
									<div class="row">
										<div class="col-md-6">
											<div class="form-group">
												...
												<div class="col-md-9">
													<p class="form-control-static">
														 <c:out value="${account.recordUser}" />
													</p>
												</div>
											</div>
										</div>
										...
										<div class="col-md-6">
											<div class="form-group">
												...
												<div class="col-md-9">
													<p class="form-control-static">
														<c:out value="${account.recordDate}" />
													</p>
												</div>
											</div>
										</div>
...
<script>
	jQuery(document).ready(function() {
		...
		if ("${cuentaHabilitada}"){
			toastr.success("${accountEnabledLabel}", "${nombreCuenta}" );
		}
		if ("${cuentaDeshabilitada}"){
			toastr.error("${accountDisabledLabel}", "${nombreCuenta}" );
		}
		
		$(".act").click(function(){ 
			$('#accionUrl').val($(this).data('id'));
        	$('#titulo').html('<h2 class="modal-title">'+"${confirmar}"+'</h2>');
        	$('#cuerpo').html('<h3>'+"${habilitar}"+' '+$(this).data('account')+'?</h3>');
        	$('#basic').modal('show');
        });
        ...
        $(".desact").click(function(){ 
        	$('#accionUrl').val($(this).data('id'));
        	$('#titulo').html('<h2 class="modal-title">'+"${confirmar}"+'</h2>');
        	$('#cuerpo').html('<h3>'+"${deshabilitar}"+' '+$(this).data('account')+'?</h3>');
        	$('#basic').modal('show');
        });
