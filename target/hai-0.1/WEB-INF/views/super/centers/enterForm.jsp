		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-md-12">
					...
					<h3 class="page-title">
					<spring:message code="supercenters" />
					</h3>
					<ul class="page-breadcrumb breadcrumb">
						<li>
							<i class="fa fa-home"></i>
							<a href="<spring:url value="/" htmlEscape="true "/>"><spring:message code="home" /></a>
							<i class="fa fa-angle-right"></i> <a href="<spring:url value="/super/centers/" htmlEscape="true "/>"><spring:message code="supercenters" /></a> <i class="fa fa-angle-right"></i> <a href="<spring:url value="/super/centers/newCenter" htmlEscape="true "/>"><spring:message code="add" /></a>
						</li>
					</ul>
			...
			<!-- END PAGE HEADER-->
			<!-- BEGIN PAGE CONTENT-->
			<spring:url value="/super/centers/newCenter" var="newCenterUrl"></spring:url>
			<spring:url value="/super/centers/" var="centerUrl"></spring:url>
			<c:set var="centerCreated"><spring:message code="process.success" /></c:set>
						...
						<form action="#" autocomplete="off" id="add-center-form" class="form-horizontal">
								<div class="form-body">
									...
									<div class="form-group">
										<label class="control-label col-md-3"><spring:message code="idCentro" />
										<span class="required">
											 *
										</span>
										</label>
										<div class="col-md-5">
											<div class="input-group">
												<input value="${center.idCentro}" id="idCentro" name="idCentro" readonly type="text" placeholder="<spring:message code="idCentro" />" class="form-control"/>
									...
									<div class="form-group">
										<label class="control-label col-md-3"><spring:message code="nombreCentro" />
										<span class="required">
											 *
										</span>
										</label>
										<div class="col-md-5">
											<div class="input-group">
												<input value="${center.nombreCentro}" id="nombreCentro" name="nombreCentro" type="text" placeholder="<spring:message code="please.enter" /> <spring:message code="nombreCentro" />" class="form-control"/>
												<span class="input-group-addon">
													<i class="fa fa-keyboard-o"></i>
												</span>
											</div>
									...
									<div class="form-group">
										...
										<div class="col-md-5">
											<select multiple="multiple" class="multi-select" id="users" name="users">
												<c:forEach items="${usuarioscentro}" var="user" varStatus="stat">
													<c:set var="usuariosCentro" value="${stat.first ? '' : usuariosCentro} ${user.userCenterId.username}" />
												</c:forEach>
												<c:forEach items="${usuarios}" var="usuario">
													<c:choose> 
														<c:when test="${fn:contains(usuariosCentro, usuario.username)}">
									...
									<div class="form-group">
										...
										<div class="col-md-5">
											<select multiple="multiple" class="multi-select" id="studies" name="studies">
												<c:forEach items="${estudioscentro}" var="study" varStatus="stat">
													<c:set var="estudiosCentro" value="${stat.first ? '' : estudiosCentro} ${study.estudioCentroId.estudio}" />
												</c:forEach>
												<c:forEach items="${estudios}" var="estudio">
													<c:choose> 
														<c:when test="${fn:contains(estudiosCentro, estudio.idEstudio)}">
															<option selected value="${estudio.idEstudio}">${estudio.nombreEstudio}</option>
														</c:when>
														<c:otherwise>
...
<script>
    ...
    function ejecutarAccionFinalizar() {
    	if($('#idCentro').val()==''){
    		...
    		window.location.href = "${centerUrl}viewCenter/"+$('#idCentro').val();
    	}
	}
