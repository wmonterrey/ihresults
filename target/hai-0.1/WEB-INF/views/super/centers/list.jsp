							...
							<c:forEach items="${centros}" var="centro">
								<tr>
									<spring:url value="/super/centers/viewCenter/{center}"
										var="centroUrl">
										<spring:param name="center" value="${centro.idCentro}" />
									</spring:url>
									...
									<td>
										...
										<c:choose>
										<c:when test="${centro.pasive=='0'.charAt(0)}">
											<a data-toggle="modal" data-centro="${centro.nombreCentro}" data-id= "${fn:escapeXml(disableUrl)}" class="btn btn-default btn-xs desact"><i class="fa fa-trash-o"></i></a>
										</c:when>
										<c:otherwise>
											<a data-toggle="modal" data-centro="${centro.nombreCentro}" data-id= "${fn:escapeXml(enableUrl)}" class="btn btn-default btn-xs act"><i class="fa fa-check"></i></a>
										</c:otherwise>
										</c:choose>
									</td>
								</tr>
